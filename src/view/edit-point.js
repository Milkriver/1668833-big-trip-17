...
const editPointTemplate = (point) => {
  ...
  const renderEventItem = (offerTypeArray) => offerTypeArray.map((offer) =>
    `<div class="event__type-item">
      <input id="event-type-${offer}-1" class="event__type-input  visually-hidden" type="radio" name="event-type" value="${offer}">
      <label class="event__type-label  event__type-label--${offer}" for="event-type-${offer}-1">${generateRandomMock(offerTypeArray)}</label>
    </div>`
  );
  const renderOfferItem = (offersArray) => offersArray.map((offer) =>
